  ...
  return (
    <div className="flex flex-col h-full min-h-0 relative">
      ...
      <div
        ...
        className={`overflow-y-auto overflow-x-hidden px-3 pt-2 pb-4 flex flex-col min-h-0 flex-1 ${store.compact ? 'gap-0.5' : 'gap-1.5'} ${!viewReady ? 'opacity-0 pointer-events-none' : ''} ${store.boardGradient && store.animatedGradient ? (store.randomGradient ? 'animate-gradient-random' : 'animate-gradient-shift') : ''}`}
        style={store.boardGradient ? {
          ...
          ...(store.animatedGradient ? { animationDuration: `${store.gradientSpeed}s` } : {}),
        } : undefined}
      >
        {messages.length > 0 ? (
          <>
